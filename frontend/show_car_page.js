...
export class ShowCarPage extends BaseWindow {
    ...
    icon_value_text(image_path, text, element = "div") {
        return `
            <${element} class="flex gap-2 m-2">
                <img src="${image_path}" alt="" class="h-8 w-8" />
    ...
    render_car_card(car_data) {
        ...
        return `
            <article class="car_card border border-gray-200 bg-white flex items-stretch rounded-xl overflow-hidden shadow-sm m-10">
                <img
                    src="assets/skoda_octavia__622641438f.jpg"
                    alt=""
                    class="w-80 h-auto object-cover shrink-0 self-stretch"
                />

                <div class="car_info flex-1 p-5 flex justify-between items-center">
                    <div class="info_about_car flex flex-col gap-4">
                    ...
                    <div class="manage_btn_and_indicators mt-4">
                ...
                <div class="car_indicator w-16 shrink-0" style="background-color: ${indicatorColor}"></div>
            </article>
        `;
    }
