...
export class GetCarPage extends BaseWindow {
  ...
  content(maintenance_enum) {
    const maintenance_type_show_values = maintenance_enum.map(
      (val) => val.name,
    );
    ...
    const fetchAndRender = async (isLoadMore = false) => {
      ...
      try {
        ...
        if (maintenance_logs && maintenance_logs.length > 0) {
          maintenances.insertAdjacentHTML(
            "beforeend",
            this.render_maintenance_log(maintenance_logs),
          );
...
`;
  ...
  async render() {
    ...
    try {
      ...
      const delete_input = document.querySelector(
        "input[name='delete_car_input']",
      );
      ...
      if (delete_car_btn && modal_overlay) {
        ...
        confirm_delete_btn?.addEventListener("click", async () => {
          if (delete_input && delete_input.value.trim() === "ВИДАЛИТИ") {
            ...
            alert(
              'Для підтвердження необхідно ввести слово "ВИДАЛИТИ" без лапок.',
            );
          }
        });
      }
